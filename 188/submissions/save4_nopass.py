...
def get_all_line_counts(data: str = STATS) -> list:
    ...
    f = open("/tmp/testfiles_number_loc.txt", "r")
    contents = f.read()
    ls = contents.split("\n")
    return [int(text.strip().split()[0]) for text in ls[:len(ls)-1]]
...
def create_stats_report(data=None):
    ...
    stats = dict(count=len(data),
                 ...
                 sample_stdev=st.pstdev(sample),
                 sample_variance=st.pvariance(sample),
                 )
